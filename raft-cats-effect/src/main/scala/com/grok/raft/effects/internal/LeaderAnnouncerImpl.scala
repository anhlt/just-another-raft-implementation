...
/** Implementation of the LeaderAnnouncer trait using Cats Effect concurrency primitives.
  *
  * This class manages leadership announcements in a Raft cluster environment. It uses a Ref to hold a Deferred promise
  * representing the current leadership announcement.
  *
  ...
  *
  * @tparam F
  *   effect type with concurrent capabilities
  */
class LeaderAnnouncerImpl[F[_]: Concurrent](ref: Ref[F, Deferred[F, NodeAddress]]) extends LeaderAnnouncer[F]:

  /** Listen for the next leader announcement. This method obtains the current Deferred and waits for its completion. If
    * the leader is already announced, the returned effect completes immediately.
    *
    * @return
    *   an effect producing the current or next leader
    */
  ...
  /** Announce a newly elected leader. Completes the current Deferred promise, unblocking all listeners waiting on
    * `listen`. If the Deferred is already completed, this call is a no-op.
    *
    * @param leader
    *   the newly elected leader node
    * @return
    *   effect signaling completion of announcement
    */
  ...
  /** Reset the announcer for a new leader. Creates a fresh Deferred promise and installs it into the Ref, so listeners
    * that call `listen` afterward will await the next announcement.
    *
    * This is typically called when observing a new term or leadership change, to invalidate previously announced
    * leaders.
    *
    * @return
    *   effect signaling completion of reset
    */
