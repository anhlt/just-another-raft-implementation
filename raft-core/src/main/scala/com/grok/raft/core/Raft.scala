...
trait Raft[F[_]] {
  ...
  def start()(using MonadError[F, Error], Logger[F]): F[Unit] = {
    for {
  ...
  def runElection()(using MonadError[F, Error], Logger[F]): F[Unit] =
    for {
      _        <- delayElection()
      logState <- log.state
  ...
  def modifyState[B](f: Node => (Node, B))(using MonadError[F, Error], Logger[F]): F[B] =
    for {
  ...
  def runActions(actions: List[Action])(using MonadError[F, Error], Logger[F]): F[Unit] =
    actions.traverse(action => runAction(action)) *> Monad[F].unit

  def runAction(action: Action)(using MonadError[F, Error], Logger[F]): F[Unit] = {
    action match {
  ...
  def onLogRequest(msg: LogRequest)(using MonadError[F, Error], Logger[F]): F[LogRequestResponse] = {
    for {
  ...
  def onLogRequestResponse(msg: LogRequestResponse)(using MonadError[F, Error], Logger[F]): F[Unit] =
    for {
  ...
  def onVoteRequest(msg: VoteRequest)(using MonadError[F, Error], Logger[F]): F[VoteResponse] = {
    for {
  ...
  def onVoteResponse(msg: VoteResponse)(using MonadError[F, Error], Logger[F]): F[Unit] =
    for {
  ...
  def scheduleElection()(using MonadError[F, Error], Logger[F]): F[Unit] = {
    background {
  ...
  def scheduleReplication()(using MonadError[F, Error], Logger[F]): F[Unit] = {
    background {
  ...
  def background[A](fa: => F[A])(using MonadError[F, Error]): F[Unit]
  ...
  def onCommand[T](c: Command[T])(using MonadError[F, Error], Logger[F]): F[T] = c match {
