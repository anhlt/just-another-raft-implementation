 ...
 *
 * When the append operation is successful, the leader updates the ackIndexMap with the provided
 * ackLogIndex value. The leader is then able to commit the message once the ackIndexMap reflects
 ...
 *   - Node 5: 5
 *
 * Given a quorum size of 3, the message can be committed if at least three nodes have an ackLogIndex
 * of 3 or higher.
 *
 * @param nodeId
