...
/** Key-Value specific state machine trait that extends the base StateMachine. Provides a pluggable interface for
  * different KV storage implementations.
  *
  ...
  */
trait KVStateMachine[F[_]](implicit raiseF: Raise[F, StateMachineError])
    extends StateMachine[F, Map[Array[Byte], Array[Byte]]] {
  ...
  /** Default implementation of applyWrite using the KV operations above Includes error handling for unknown commands
    */
  ...
  /** Default implementation of applyRead using the KV operations above Includes error handling for unknown commands
    */
