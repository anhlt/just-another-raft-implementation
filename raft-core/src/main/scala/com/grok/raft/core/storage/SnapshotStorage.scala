...
trait SnapshotStorage[F[_], T] {
   ...
   * @return A monadic effect that completes when the snapshot is persisted.
   */
   ...
   *
   * @return A monadic effect containing an optional snapshot.
   */
   ...
   * Get latest snapshot
   *
   * @return A monadic effect containing the latest snapshot.
   */
