...
/** Log replication request from leader to follower in the Raft consensus algorithm.
  ...
  *
  * @param leaderId
  *   Leader's unique identifier
  * @param term
  *   Leader's current term
  * @param prevSentLogIndex
  *   Index of the last log entry previously sent to this follower (0-based)
  * @param prevLastLogTerm
  *   Term of the log entry preceding new entries
  * @param leaderCommit
  *   Leader's last committed log index
  * @param entries
  *   Log entries to replicate
  *
  * Leaders track sent entries using a sentIndex mapping for each follower. prevLastLogTerm enables followers to verify
  * log consistency.
  */
