    ...
    Handle
      .allow[RaftError] {
        Handle
          .allow[LogError] {
            Handle
              .allow[StateMachineError](operation)
              .rescue {
                case StateMachineError.InvalidCommand(commandType) =>
                  IO.raiseError(new RuntimeException(s"Invalid command: $commandType"))
                case error =>
...
class InMemoryKVStateMachine[F[_]](implicit syncF: Sync[F], raiseF: Raise[F, StateMachineError])
    extends KVStateMachine[F] {
  ...
  override def restoreSnapshot[T](lastIndex: Long, data: T): F[Unit] = {
    try {
      ...
      case _: ClassCastException =>
        Sync[F].raiseError(
          new RuntimeException("StateMachineError: snapshot data is not a valid Map[Array[Byte], Array[Byte]]")
        )
      case e: Exception =>
