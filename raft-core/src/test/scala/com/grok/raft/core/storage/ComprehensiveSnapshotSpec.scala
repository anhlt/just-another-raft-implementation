  ...
  test("Bank state machine complex read/write operations with snapshots") {
    withStateMachineErrorHandling {
      for {
        ...
        transferSuccess <- log.stateMachine
          .applyWrite((6, Transfer("bob", "charlie", 100.0)))
        ...
        restoredCharlieBalance <- newLog.stateMachine
          .applyRead(GetBalance("charlie"))
          .map(_.asInstanceOf[Option[Double]])
  ...
  test("Enhanced KV state machine with comprehensive read/write operations") {
    withStateMachineErrorHandling {
      for {
        ...
        rangeResults <- kvSM
          .applyRead(Range(startKey, endKey, None))
          .map(_.asInstanceOf[List[(Array[Byte], Array[Byte])]])
  ...
  test("Large state machine snapshot and restoration") {
    withStateMachineErrorHandling {
      for {
        ...
        prefixResults <- kvSM
          .applyRead(Scan("large_key_01".getBytes, Some(5)))
          .map(_.asInstanceOf[List[(Array[Byte], Array[Byte])]])
